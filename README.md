# Volný projekt

> **Jméno a příjmení:** `Roman Sedlář`

> **Název projektu:** `TheRainShark portfolio`

---

## O projektu

`Portfolio, ostatní mají možnost mi napsat nebo zažádat o hudbu - nemusím jim zbytečne posílat všechno co jsem udělal, stačí jen ten web a je to.`

## Konkurence / Inspirace

Moje inspiracia: 
 [Filip Znachor](https://f-z.cz/)
 [thefatrat.com](https://www.thefatrat.com/)
 [incompetech.com](https://incompetech.com/music/royalty-free/music.html)

## Cílová skupina

`Zájemci o Hudbu pro filmy, hry, videa...`

---

## Odkazy a výstupy

| Výstup | Odkaz |
|---|---|
| GitHub Pages (live) | [`>ODKAZ NA WEB TADY<`](https://pslib-cz.github.io/2025-p2a-web-volny-projekt-RomanSedlo/) |
| Grafický manuál (Figma / Canva) | [`Figma Design`](https://www.figma.com/design/98TS964aiMU6INuBdVLD2q/Sedl%C3%A1%C5%99Roman-TheRainShark-Portfolio?node-id=0-1&t=OqKzTWe66Wb6MKp1-1) |
| Grafický návrh webu (Figma) | [`Figma, mobile and PC design`](https://www.figma.com/design/98TS964aiMU6INuBdVLD2q/Sedl%C3%A1%C5%99Roman-TheRainShark-Portfolio?node-id=9-2&t=OqKzTWe66Wb6MKp1-1) |
| Fotografie / média | [`disk X (takže zatím nefunguje)`](file:///X:/MME/web-projekt/gallery/) |

---

## Poznámky

_Prostor pro vlastní poznámky, zpětnou vazbu, TODO apod._
